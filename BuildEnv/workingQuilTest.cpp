...
int main(){
  ...
  //~~~~~~~~~~~~ CONFIG INPUTS ~~~~~~~~~~~~~~~~~~~~
    ...
    const uint32_t INITIAL_SEED = 0x4949F84F; 
    ...
    for (unsigned int i = 0;i < npcSet.size(); i++){
      //Two steps are taken for NPCs before any other RNG rolls.
      npcSet[i].npcAction_Self(seed); //Using state FIRST
      //If using a manager and given a seed to that manager, then can sim internally, saves the programmer having to remember this step.
    }

...
d_coord vectorScale(double factor, d_coord a){
    return {a.x*factor, a.z*factor, a.y*factor}; //Note not the same as vectorMultiply as that would imply I take two vecs and multiply them together.
}
d_coord vectorSquare(d_coord a){ //helper 
    return {a.x*a.x, a.z*a.z, a.y*a.y}; 
    //alt: return {pow(a.x,2),pow(a.z,2),pow(a.y,2)};
}
double vectorCombine(d_coord a){ //helper
    return {a.x+a.z+a.y};
}

double vectorSquareDistance(d_coord a, d_coord b){
    return pow((a.x-b.x),2) + pow(a.z-b.z,2)+ pow(a.y-b.y,2);
    //return vectorCombine(vectorSquare(vectorSub(a,b)))
}
double vectorDistance(d_coord a, d_coord b){ //same as pythag distance but this makes no assumptions about the input data -_- DOES CONSIDER Z
    //return sqrt(pow((a.x-b.x),2) + pow((a.z-b.z),2) + pow((a.y-b.y),2)); //Will never be a negative result.
    return sqrt(vectorSquareDistance(a,b));
}
double vectorSquareMag(d_coord in){
    return (in.x*in.x) + (in.z * in.z) + (in.y*in.y);
//  return (vectorCombine(vectorSquare(in)));
}
double vectorMagnitude(d_coord in){
    ...
    return sqrt(vectorSquareMag(in));
}

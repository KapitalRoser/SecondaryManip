...
d_coord vectorAdd(d_coord a, d_coord b){
    return {a.x+b.x,a.y+b.y}; //Not an idiot, in the code this function does not consider z!
}
...
d_coord vectorCopy (const d_coord& in){
...
int GS_collision(int ballSize, d_coord& adjustedPositions_probably){
    ...
    divResult = divResult >= 1 ? 1 : divResult; //Forces div result to be between 0 and 1 inclusive.
     ...
     //In our case, will never be anything other than 1.
    //Would need a step distance of >ballsize, like a 3 or 4, which will never happen. We only take steps of size 0.1 and such.
    //Therefore divResult will be massive, and then immediately capped at 1.

    d_coord subResult = vectorSub(proposed,old);

    for (int i = 0; i < 1; i+= divResult){
        //DivResult is only ever 1 in our case, and since this loop is max 1, then we can get rid of the loop structure.
